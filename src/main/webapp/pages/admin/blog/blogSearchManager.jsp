        ...
        <div class="main">
            ...
            <table id="table" class="table color2">
                ...
                <c:forEach items="${blog_list}" var="blog" varStatus="status">
                    <tr>
                        <td class="checkbox"><input name="id" type="checkbox" value="${blog.id}" class="fill listen-1-2"/> </td>
                        <td style="text-align: center">${blog.id}</td>
                        <td style="text-align: center">${blog.title}</td>
                        <td style="text-align: center">${blog.releaseDateStr}</td>
                        <td style="text-align: center">${blog.blogType.typeName}</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
...
<script type="text/javascript">
    ...
    function deleteInfo() {
        ...
        console.log(idArr);
        $.ajax({
            url : "${pageContext.request.contextPath}/admin/blog/blogDelete.do",
            type : "POST",
            dataType : "json",
            traditional : "true",
            data : {
